...
- **typescript/**: `@bennyblader/ddk` (formerly ddk-ts) — Node (generated TypeScript over
...
compiled cdylib, and those scripts are gone. Do not add Rust to `typescript/`; add
...
### Node + Browser Library Commands (run from typescript/)

- `pnpm generate`: Build the ddk-ffi cdylib, generate `src/node/generated/`,
...
3. **Regenerate ddk too**: `just ts-build` and `just ts-wasm-build` (or
   `pnpm generate` / `pnpm build:wasm` in `typescript/`). `typescript/src/node/generated` and
   `typescript/src/wasm/generated` are generated *and committed*, so a Rust change
...
- `typescript/src/{node,wasm}/generated/`: Generated TypeScript bindings (committed; never hand-edited)
- `typescript/src/{node,wasm}/index.ts`: the two hand-written entries; `typescript/package.json`'s
  `exports` picks one (`node` → N-API, `browser`/`default` → wasm, `./wasm` → wasm)
- `typescript/scripts/build-release.mjs`: cargo build → generate → tsc → platform packages
- `typescript/scripts/build-wasm.mjs`: `ubrn build wasm2` → tsc → stage the .wasm
- `typescript/scripts/publish-release.mjs`: platform packages first, then the main package
- `typescript/ubrn.config.yaml`: N-API wiring (documentation) + the wasm2 config (read by ubrn)
...
- TypeScript bindings in `ddk-rn/src/` **and `typescript/src/*/generated/`**
...
(`ddk-rn/package.json`), the `@ubjs/core`, `@ubjs/node` and `@ubjs/wasm` runtimes, and the
...
It sets the version in `typescript/package.json`, `ddk-rn/package.json` and
...
the `.wasm` inside the main package. Details in `typescript/DEVELOPMENT.md`; the parts
that bite:

- **On Node it is N-API or nothing.** `src/node/index.ts` rethrows a missing
...
- Everything installs from npm: the BAL party is the **latest published
  ...
  under the **`bal-ddk-ts` alias** (it must coexist with `link:../typescript`),
...
- **The full suite runs in CI, in the `check` job** — including the regtest
  ...
  `compat/` resolves it through `link:../typescript`. Ubuntu has no bitcoind
...
- **typescript/CHANGELOG.md**: For `@bennyblader/ddk` (Node + browser) changes
