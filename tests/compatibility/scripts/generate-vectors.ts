 ...
 *
 *   - tests/compatibility/vectors/compat-vectors.json          (this package's guard suite)
 *   - examples/react-native/src/compatVectors.ts         (replayed on-device by Maestro)
 *
...
const vectors: CompatVectors = {
  meta: {
    generatedBy: 'tests/compatibility/scripts/generate-vectors.ts',
    ddkTsVersion: ddk.version(),
    nodeDlcVersion: '1.2.1',
    note: 'GENERATED — regenerate with `pnpm vectors` in tests/compatibility/. Every message was round-tripped and validated against @node-dlc at generation time.',
  },
  offerer: {
...
// --- write outputs ---
const vectorsDir = resolve(REPO_ROOT, 'tests', 'compatibility', 'vectors')
...
const rnModule = `/**
 * GENERATED by tests/compatibility/scripts/generate-vectors.ts — do not edit by hand.
 * Regenerate with \`pnpm vectors\` in tests/compatibility/ after a Rust core change.
 *
...
`
writeFileSync(resolve(REPO_ROOT, 'examples', 'react-native', 'src', 'compatVectors.ts'), rnModule)
...
const replaySource = readFileSync(resolve(REPO_ROOT, 'tests', 'compatibility', 'src', 'replay.ts'), 'utf8')
writeFileSync(
  resolve(REPO_ROOT, 'examples', 'react-native', 'src', 'compatReplay.ts'),
  `// GENERATED — verbatim copy of tests/compatibility/src/replay.ts, written by\n// tests/compatibility/scripts/generate-vectors.ts (\`pnpm vectors\`). Do not edit here.\n${replaySource}`,
)
...
console.log(`  ${resolve(REPO_ROOT, 'examples', 'react-native', 'src', 'compatVectors.ts')}`)
