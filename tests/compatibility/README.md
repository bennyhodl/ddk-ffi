...
- `scripts/generate-vectors.ts` produces a deterministic transcript (a
  ...
  **every message against @node-dlc at generation time**, and writes it to
  `vectors/compat-vectors.json` and `examples/react-native/src/compatVectors.ts`,
  along with a verbatim copy of the replay code
  (`examples/react-native/src/compatReplay.ts` — `src/replay.ts` is dependency-free
...
- The example app's **"Run the BAL compat replay"** button executes it through
  ...
  `examples/react-native/.maestro/compat-flow.yaml` asserts it in the E2E suite
  (`just native e2e-ios` / `just native e2e-android`).
...
```sh
just install        # once; everything comes from npm
just build node              # the suite tests ddk's current dist
just test compatibility           # everything (spawns a throwaway regtest bitcoind)
just compat-messages  # offline suites only
...
which is what `resolveLibPath()` resolves through. `tests/compatibility/` reaches that via
`link:../../packages/node-browser`, so CI tests the ddk built from the commit under test, while
