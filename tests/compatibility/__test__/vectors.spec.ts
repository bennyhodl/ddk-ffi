...
describe('committed compat vectors', () => {
  ...
  test('the ddk-rn copy of the vectors is in sync', () => {
    const rnModule = readFileSync(resolve(REPO_ROOT, 'examples', 'react-native', 'src', 'compatVectors.ts'), 'utf8')
  ...
  test('the ddk-rn copy of the replay is in sync', () => {
    ...
    const rnCopy = readFileSync(resolve(REPO_ROOT, 'examples', 'react-native', 'src', 'compatReplay.ts'), 'utf8')
