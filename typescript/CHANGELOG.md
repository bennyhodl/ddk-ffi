# @bennyblader/ddk Changelog

Formerly `@bennyblader/ddk-ts`; entries before the rename keep the old name.
...
A WebAssembly build of the same `ddk-ffi` crate, generated by ubrn's wasm2 target and shipped in this package. Same API as the native entry; call `await init()` once first. It runs in browsers with no COOP/COEP headers. `ddk_ffi.wasm` is 4.8MB (2.9MB gzipped).
...
- **The browser build is WebAssembly from ubrn's wasm2 target.** The WASI/emnapi fallback (`@bennyblader/ddk-ts-wasm32-wasi`) went with napi-rs. Bundlers pick the replacement through the `browser` export condition; `await init()` before the first call.
...
Nothing is persisted. Every transaction is rebuilt from the offer/accept/sign wire messages at the moment it is needed, so there is no contract store to keep in sync, and funding secret keys stay in Rust behind a `ContractKeyProvider` and never cross into JS. Either party can settle on its own. Built on the published `ddk` / `ddk-dlc` 2.0.0-rc.6 crates, and in parity with `@bennyblader/ddk-rn`.
