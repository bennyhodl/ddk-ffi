...
```bash
# Build the N-API bindings for current platform
just ts-build
...
# The wasm build, its tests, and the headless-Chrome check
just ts-wasm-build
just ts-wasm-test
just ts-wasm-smoke
...
├── src/
│   ├── node/
│   │   ├── index.ts    # the `node` entry (hand-written): throws without a native lib
│   │   └── generated/  # ubrn N-API output — committed, never hand-edited
│   └── wasm/
│       ├── index.ts    # the browser entry (hand-written): init() finds the .wasm
│       └── generated/  # ubrn wasm2 output — committed (the .wasm itself is not)
├── dist/               # tsc output: dist/node/ + dist/wasm/ (shipped in the tarball)
├── platform/           # one npm package per target (git-ignored)
│   └── darwin-arm64/   # package.json + libddk_ffi.dylib
├── example/            # Example Node application
├── example-browser/    # Vite app + headless-Chrome smoke test
├── __test__/           # vitest suites — run against both bindings
└── scripts/
    ├── build-release.mjs   # N-API: cargo build → generate → tsc → platform packages
    ├── build-wasm.mjs      # wasm: ubrn build wasm2 → tsc → stage the .wasm
...
`src/*/generated/` still matches the crate.
