...
`typescript/` contains **no Rust and no hand-written bindings**. It is generated from
the compiled `ddk-ffi` crate by `uniffi-bindgen-react-native` — twice: the N-API
target for Node (`src/node/generated/`) and the wasm2 target for browsers
(`src/wasm/generated/`). The same bindgen and crate are behind
...
`src/node/index.ts` and `src/wasm/index.ts`, and the `exports` map in
...
2. **`pnpm generate`** and **`pnpm build:wasm`** — regenerate
   `src/node/generated/` and `src/wasm/generated/` from the new library.
...
uniffi-bindgen-react-native generate napi bindings \
  --library \
  --ts-dir ../typescript/src/node/generated \
...
`src/wasm/generated/`. That TypeScript is committed and diffed in CI, like the
N-API output; the `.wasm` is not. `src/wasm/index.ts` is the hand-written entry:
...
That bumps `typescript/package.json`, `ddk-rn/package.json`, `ddk-ffi/Cargo.toml` and
