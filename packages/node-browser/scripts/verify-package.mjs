#!/usr/bin/env node
/**
 * Refuse to publish an @bennyblader/ddk tarball that cannot load a native library.
 *
...
if (!existsSync(join(PKG, 'dist', 'node', 'index.js')))
  problems.push('dist/node/index.js is missing (nothing was built)')
if (!existsSync(join(PKG, 'dist', 'node', 'index.d.ts'))) problems.push('dist/node/index.d.ts is missing (no types)')
if (!existsSync(join(PKG, 'dist', 'wasm', 'index.js')))
  problems.push('dist/wasm/index.js is missing (no browser entry)')
if (!existsSync(join(PKG, 'dist', 'wasm', 'generated', 'ddk_ffi.wasm'))) {
  problems.push('dist/wasm/generated/ddk_ffi.wasm is missing — the browser entry would install and fail in init()')
}
...
if (problems.length > 0) {
  ...
  console.error('\nRelease with: node scripts/publish-release.mjs (see packages/node-browser/DEVELOPMENT.md)\n')
  process.exit(1)
}

console.log(
  `✅ ${manifest.name}@${manifest.version} — dist/node + dist/wasm present, ${names.length} platform package(s) referenced`,
)
