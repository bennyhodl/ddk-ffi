# @bennyblader/ddk Development

`packages/node-browser/` contains **no Rust and no hand-written bindings**. It is generated from
the compiled `ddk-ffi` crate by `uniffi-bindgen-react-native` — twice: the N-API
target for Node (`node/generated/`) and the wasm2 target for browsers
(`browser/generated/`). The same bindgen and crate are behind
`@bennyblader/ddk-rn`. Generated output is committed, and CI fails if it does
not match the crate. The only hand-written sources are the two entries,
`node/index.ts` and `browser/index.ts`, and the `exports` map in
`package.json` that picks between them (`node` → N-API; `browser` and `default`
→ wasm; `./wasm` → wasm explicitly).
...
pnpm test          # vitest, against dist/node/
```
...
1. **Change `ffi/src/*.rs`** — the single source of truth, annotated with
   `#[uniffi::export]` / `#[derive(uniffi::Record)]`.
2. **`pnpm generate`** and **`pnpm build:wasm`** — regenerate
   `node/generated/` and `browser/generated/` from the new library.
3. **`pnpm test`** and **`pnpm test:wasm`**.
4. **Commit both `generated/` directories** along with the Rust change. A `ddk-ffi` change without its
...
uniffi-bindgen-react-native generate napi bindings \
  --library \
  --ts-dir ../packages/node-browser/node/generated \
  --lib-package-base '@bennyblader/ddk-' \
  --lib-node-triple \
  "$PWD/target/release/libddk_ffi.dylib"
```

`scripts/build-release.mjs` runs exactly this, then patches the ESM specifiers,
compiles with `tsconfig.node.json`, and assembles the platform packages. Three
...
- **`--lib-package-base` bakes in the resolver.** The generated code calls
  `resolveLibPath({ npmPackageBase: '@bennyblader/ddk-', tripleStyle: 'node' })`,
  which does `require.resolve('@bennyblader/ddk-darwin-arm64/package.json')`
  at runtime. No absolute paths are baked in.

The bindings _config_ — `strictByteArrays`, `logLevel` — is not passed here. Ubrn
auto-discovers `ffi/uniffi.toml` and deliberately ignores `--config`, so that
file governs the Node, wasm and React Native bindings together. Changing
...
```
@bennyblader/ddk/                    main package (ESM)
  dist/node/   index.js + generated/  (the `node` condition)
  dist/wasm/   index.js + generated/ + ddk_ffi.wasm  (`browser`, `default`, `./wasm`)
  package.json   optionalDependencies -> every platform package

@bennyblader/ddk-darwin-arm64/       one per target
...
- **Node triples, not cargo triples**: `darwin-arm64`, `linux-x64-gnu`,
  `win32-x64-msvc` — the same suffixes ddk-ts published, under the new
  `@bennyblader/ddk-` prefix.

Locally, `pnpm generate` symlinks `platform/<triple>` into
`node_modules/@bennyblader/ddk-<triple>`, so tests and the example resolve the
...
pnpm test:browser:smoke   # Vite build, loaded in headless Chrome
```
...
`browser/generated/`. That TypeScript is committed and diffed in CI, like the
N-API output; the `.wasm` is not. `browser/index.ts` is the hand-written entry:
it adds `init()`, which defaults the module's location.
...
That bumps `packages/node-browser/package.json`, `packages/react-native/package.json`, `ffi/Cargo.toml` and
its lockfile, commits, tags and pushes. Pushing the tag is what publishes.

CI builds one cdylib per platform on its own runner, and the wasm module once,
then runs
