...
### Browser support: `@bennyblader/ddk-ts/wasm`

A WebAssembly build of the same `ddk-ffi` crate, generated by ubrn's wasm2 target and shipped in this package. Same API as the native entry; call `await init()` once first. It runs in browsers with no COOP/COEP headers, and in Node on platforms with no native binary. `ddk_ffi.wasm` is 4.8MB (2.9MB gzipped).
...
- **The browser build moved to `@bennyblader/ddk-ts/wasm`.** The WASI/emnapi fallback (`@bennyblader/ddk-ts-wasm32-wasi`) went with napi-rs. Its replacement is not picked up through the `browser` field: import `@bennyblader/ddk-ts/wasm` and `await init()` before the first call.
