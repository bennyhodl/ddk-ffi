...
Nothing is persisted. Every transaction is rebuilt from the offer/accept/sign wire messages at the moment it is needed, so there is no contract store to keep in sync, and funding secret keys stay in Rust behind a `ContractKeyProvider` and never cross into JS. Either party can settle on its own. Built on the published `ddk` / `ddk-dlc` 2.0.0-rc.3 crates, and in parity with `@bennyblader/ddk-rn`.
